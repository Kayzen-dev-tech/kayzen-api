...
function checkAuth() {
    const token = localStorage.getItem(config.setting.adminToken);
    if (token) {
        isAdmin = true;
        updateAdminUI();
    }
...
async function doLogin() {
    ...
    try {
        ...
        if (res.status) {
            localStorage.setItem(config.setting.adminToken, res.token);
            isAdmin = true;
...
function logout() {
    localStorage.removeItem(config.setting.adminToken);
    isAdmin = false;
    location.reload();
}
...
async function deleteCode(e, id) {
    ...
    try {
        const req = await fetch(`/api/admin/delete/${id}`, {
            method: 'DELETE',
            headers: { 'Authorization': localStorage.getItem(config.setting.adminToken) }
        });
        const res = await req.json();
        if(res.status) {
