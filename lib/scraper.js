...
async function youtubeDl(url) {
  try {
    const { data: init } = await axios.post('https://www.y2mate.com/mates/analyzeV2/ajax', new URLSearchParams({ k_query: url, k_page: 'home', hl: 'en', q_auto: 0 }), {
      headers: { 'User-Agent': UA, 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' }
    });

    if (!init.links || !init.links.mp4) return { status: false, message: "Video not found" };
    ...
    const { data: convert } = await axios.post('https://www.y2mate.com/mates/convertV2/index', new URLSearchParams({ vid: init.vid, k: k }), {
      headers: { 'User-Agent': UA, 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' }
    });

    if (convert.status !== 'ok') return { status: false, message: "Conversion failed" };

    return { status: true, title: convert.title, download: convert.dlink, quality: convert.fquality };
  } catch (e) { return { status: false, message: "YouTube DL Error" }; }
}
...
async function ytmp3(url) {
  try {
    ...
    if (!init.links || !init.links.mp3) return { status: false, message: "Audio not found" };
    ...
    return { status: true, title: convert.title, download: convert.dlink, quality: '128kbps' };
  } catch (e) { return { status: false, message: "YouTube Audio Error" }; }
}

async function instaDl(url) {
  try {
    const { data } = await axios.post('https://snapinsta.app/action.php', new URLSearchParams({ url: url, action: 'post' }), {
      headers: { 'User-Agent': UA, 'Origin': 'https://snapinsta.app', 'Referer': 'https://snapinsta.app/' }
    });

    if (!data) return { status: false, message: "Failed to fetch Instagram" };
  ...
  } catch (e) { return { status: false, message: "Instagram DL Error" }; }
}
...
async function instaStalk(username) {
  ...
  } catch (e) { return { status: false, message: "Stalk Error" }; }
}
...
async function pinterestSearch(query) {
  try {
    ...
    const { data } = await axios.get('https://www.pinterest.com/resource/BaseSearchResource/get/', { params: params, headers: { 'User-Agent': UA, 'Referer': 'https://www.pinterest.com/' } });
    
    const results = data.resource_response.data.results.map(v => v.images.orig.url);
    return results.slice(0, 10);
  } catch (e) { return { status: false, message: "Pinterest API changed or busy" }; }
}
...
async function pinterestDl(url) {
  try {
    ...
    if (videoUrl) {
      return { status: true, type: 'video', url: videoUrl.replace('/hls/', '/720p/').replace('.m3u8', '.mp4') };
    }
...
async function twitterDl(url) {
  ...
  } catch (e) { return { status: false, message: "Twitter DL Error" }; }
}
...
async function twitterStalk(username) {
    ...
    } catch(e) { return { status: false, message: "Nitter instance busy" }; }
}
